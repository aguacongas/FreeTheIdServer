...
namespace Aguacongas.FreeTheIdServer.BlazorApp
{
    public partial class App
    {
        ...
        private Task OnNavigateAsync(NavigationContext args)
        {
            ...
            {
                return LoadAssemblyAsync("Aguacongas.FreeTheIdServer.BlazorApp.Pages.Api.wasm");
            }

            if (path == "identityresource")
            {
                return LoadAssemblyAsync("Aguacongas.FreeTheIdServer.BlazorApp.Pages.Identity.wasm");
            }

            var pageKind = _pageKindList.FirstOrDefault(k => path == $"{k.ToLower()}s");
            if (pageKind != null)
            {
                return LoadAssemblyAsync($"Aguacongas.FreeTheIdServer.BlazorApp.Pages.{pageKind}s.wasm");
            }

            pageKind = _pageKindList.FirstOrDefault(k => path.Equals(k, StringComparison.InvariantCultureIgnoreCase));
            return pageKind != null ? LoadAssemblyAsync($"Aguacongas.FreeTheIdServer.BlazorApp.Pages.{pageKind}.wasm") : Task.CompletedTask;
        }
