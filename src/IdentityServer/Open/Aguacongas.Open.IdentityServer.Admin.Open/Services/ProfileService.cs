...
public class ProfileService<TUser>(UserManager<TUser> userManager,
    IUserClaimsPrincipalFactory<TUser> claimsFactory,
    IEnumerable<IProvideClaims> claimsProviders,
    ILogger<ProfileService<TUser>> logger) : AspNetIdentity.ProfileService<TUser>(userManager, claimsFactory, logger) where TUser : class
{
    private readonly IEnumerable<IProvideClaims> _claimsProvider = claimsProviders ?? throw new ArgumentNullException(nameof(claimsProviders));
    ...
    protected virtual async Task<ClaimsPrincipal> GetClaimsPrincipalAsync(TUser user)
    {
        ...
        return principal ?? throw new InvalidOperationException("ClaimsFactory failed to create a principal");
    }
...
#pragma warning restore S3885 // "Assembly.Load" should be used
    ...
    private Task<IEnumerable<Claim>> GetClaimsFromResource(Resource resource, ClaimsPrincipal subject, Client application, string caller)
    {
        ...
        return GetClaimsFromResource(resource, subject, application, caller, providerTypeName);
    }
