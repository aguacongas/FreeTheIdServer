...
public class SignInValidator(IClientStore clients, IRelyingPartyStore relyingParties) : ISignInValidator
{
    private readonly IClientStore _clients = clients ?? throw new ArgumentNullException(nameof(clients));
    private readonly IRelyingPartyStore _relyingParties = relyingParties ?? throw new ArgumentNullException(nameof(relyingParties));
    ...
    /// <returns></returns>
    public async Task<SignInValidationResult> ValidateAsync(WsFederationMessage message, ClaimsPrincipal user, CancellationToken cancellationToken)
    {
