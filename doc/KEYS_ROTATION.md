...
"IdentityServer": {
  "Key": {
    ...
    "KeyProtectionOptions": {
      ...
      "X509CertificatePath": "C:\\certificates\\FreeTheIdServer.pfx",
...
For *EntityFramework* storage king, keys are store in the **KeyRotationKeys** table of FreeTheIdServer database
...
> **⚠️ Important Update**: FreeTheIdServer now uses the modern `Azure.Security.KeyVault.Keys` SDK. The old `Microsoft.Azure.KeyVault` SDK is obsolete.
...
The section *RsaEncryptorConfiguration* congrols the default RSA key generation options. It's binded to [`RsaEncryptorConfiguration`](../src/IdentityServer/Open/Aguacongas.Open.IdentityServer.KeysRotation.Open#rsaencryptorconfiguration-properties) 
