...
[*sample/MultiTiers/Aguacongas.FreeTheIdServer.Public/Dockerfile.Open-public*](../../sample/MultiTiers/Aguacongas.FreeTheIdServer.Public/Dockerfile.Open-public) illustrates how to create an image from the [server image](https://hub.docker.com/r/aguacongas/FreeTheIdServer) to run a public Linux server container.
...
A full deployment guide is available for hosting FreeTheIdServer on the Render platform using the provided `render.yaml` blueprint and Docker. This guide covers the initial deployment, URL configuration, and setting up persistent storage with either Render PostgreSQL or Neon.
...
The template "FreeTheIdServer.Open" was created successfully.
...
> The sample use IS4 version but you just need to remplace IS4 by Open in package reference to use the Open version.
...
The site name is defined by *SiteOptions:FreeTheIdServer*.
...
The section *AccountOptions* is bound to [`AccountOptions`](../src/Aguacongas.FreeTheIdServer.Open/Quickstart/Account/AccountOptions.cs).
...
"UpgradePasswordHasherOptions": {
    "HashPrefixMaps": {
      ...
      "162": "Aguacongas.FreeTheIdServer.Identity.Argon2PasswordHasher.Argon2PasswordHasher",
      "12": "Aguacongas.FreeTheIdServer.Identity.ScryptPasswordHasher.ScryptPasswordHasher",
      "188": "Aguacongas.FreeTheIdServer.Identity.BcryptPasswordHasher.BcryptPasswordHasher"
    },
    "UsePasswordHasherTypeName": "Aguacongas.FreeTheIdServer.Identity.Argon2PasswordHasher.Argon2PasswordHasher"
}
```

## Configure Open.IdentityServer

The section **IdentityServerOptions** is bound to the class [`Open.IdentityServer.Configuration.IdentityServerOptions`](https://docs.Opensoftware.com/identityserver/v5/reference/options/).  
So you can set any Open.IdentityServer options you want from configuration (but key management options).
...
```json
"ConnectionStrings": {
  "DefaultConnection": "Data Source=(LocalDb)\\MSSQLLocalDB;database=FreeTheIdServer;trusted_connection=yes;"
}
```
...
```json
"ConnectionStrings": {
  "DefaultConnection": "mongodb+srv://FreeTheIdServer:FreeTheIdServerpwd@cluster0.fvkfz.mongodb.net/FreeTheIdServer?retryWrites=true&w=majority"
}
```
...
"PrivateServerAuthentication": {
  "Authority": "https://FreeTheIdServerprivate",
  "ApiUrl": "https://FreeTheIdServerprivate/api",
  "ClientId": "public-server",
  "ClientSecret": "84137599-13d6-469c-9376-9e372dd2c1bd",
  "Scope": "FreeTheIdServeradminapi",
  "HttpClientName": "is4"
},
"SignalR": {
  "HubUrl": "https://FreeTheIdServerprivate/providerhub"
  "HubOptions": {
...
...
 "Users": [
    {
      "UserName": "alice",
      "Email": "alice@FreeTheIdServer.com",
      "EmailConfirmed": true,
...
...
  "Apis": [
    {
      "Name": "FreeTheIdServeradminapi",
      "DisplayName": "FreeTheIdServer admin API",
      "UserClaims": [
        "name",
        "role"
      ],
...
> The api **FreeTheIdServeradminapi** is required for the admin app.
...
> The scope **FreeTheIdServeradminapi** is required for the admin app.
...
...
  "Clients": [
    {
      "ClientId": "FreeTheIdServeradmin",
      "ClientName": "FreeTheIdServer admin SPA Client",
...
> The client **FreeTheIdServeradmin** is required by the admin app.
...
FreeTheIdServer can be configured with a keys rotation mechanism instead of a single key.  
...
By default, the issuer for the 2fa authenticator is **Aguacongas.FreeTheIdServer**.  
To update this value, set **AuthenticatorIssuer** with your issuer.

```json
"AuthenticatorIssuer": "FreeTheIdServer"
```
...
"ApiAuthentication": {
  ...
  "ApiName": "FreeTheIdServeradminapi",
...
"SwaggerUiSettings": {
  "Path": "/api/swagger",
  "OAuth2Client": {
    "ClientId": "FreeTheIdServer-swagger",
    "AppName": "FreeTheIdServer Swagger UI",
...
```json
{
    ...
    "redirect_uris": [
        "https://www.certification.openid.net/test/a/FreeTheIdServer/callback"
    ],
    "contacts": [
...
"RedisConfigurationOptions": {
  ...
  "HashKey": "Aguacongas.FreeTheIdServer.Open",
  "Channel": "Aguacongas.FreeTheIdServer.Open.Channel"
}
```
