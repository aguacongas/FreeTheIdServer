# 🚀 Deployment Guide: FreeTheIdServer on Render

This guide outlines the steps to deploy **FreeTheIdServer** directly from the public repository using the `render.yaml` blueprint. The process is divided into three stages to ensure the service URL is correctly configured and data persistence is established.
...
Once the service is created, Render assigns it a public URL (e.g., `https://FreeTheIdServer-xyz.onrender.com`). You must now update the environment variables to replace the placeholders.
...
1.  Return to your **FreeTheIdServer** Web Service -> **Environment**.
