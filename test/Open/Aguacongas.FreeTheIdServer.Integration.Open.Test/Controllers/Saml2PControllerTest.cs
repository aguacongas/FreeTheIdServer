...
public class Saml2PControllerTest(FreeTheIdServerFactory factory)
{
    ...
    public async Task Login_should_return_saml2_redirect_when_user_and_use_acs_artifact_found()
    {
        ...
        await context.Clients.AddAsync(new Client
        {
            ...
            [
                ...
                {
                    Id = Guid.NewGuid().ToString(),
                    Key = nameof(Aguacongas.Open.IdentityServer.Saml2p.Open.Services.Store.RelyingParty.UseAcsArtifact),
                    Value = true.ToString()
                }
            ]
        });
        ...
        await context.RelyingParties.AddAsync(new Aguacongas.Open.IdentityServer.Store.Entity.RelyingParty
        {
            Id = "test",
    ...
    public async Task Artifact_should_validate_request()
    {
        ...
        relyingPartyStore.Setup(m => m.FindRelyingPartyAsync(issuer)).ReturnsAsync(() =>
        {
            ...
            called = true;
            return new Aguacongas.Open.IdentityServer.Saml2p.Open.Services.Store.RelyingParty
            {
                Issuer = issuer,
                UseAcsArtifact = true,
