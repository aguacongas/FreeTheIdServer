...
using Xunit;
using UserPage = Aguacongas.FreeTheIdServer.BlazorApp.Pages.User.User;

namespace Aguacongas.FreeTheIdServer.IntegrationTest.BlazorApp.Pages
{
    [Collection(BlazorAppCollection.Name)]
    public class UserTest : EntityPageTestBase<UserPage, FreeTheIdServer.BlazorApp.Models.User>
    {
        public override string Entity => "user";

        public UserTest(FreeTheIdServerFactory factory) : base(factory)
        {
        }
        ...
        public async Task AddUserClaim_should_add_claim_to_user()
        {
            ...
            await inputList.First().ChangeAsync(new ChangeEventArgs { Value = expected });

            rows = component.FindAll("#claims tr");
            lastRow = rows[rows.Count - 1];
            inputList = lastRow.QuerySelectorAll("input");

            await inputList.Last().ChangeAsync(new ChangeEventArgs { Value = expected });
        ...
        public async Task SaveClicked_create_new_user()
        {
            ...
            await input.ChangeAsync(new ChangeEventArgs { Value = userId });
        ...
        public async Task SaveClicked_should_update_user()
        {
            ...
            await input.ChangeAsync(new ChangeEventArgs { Value = expected });
