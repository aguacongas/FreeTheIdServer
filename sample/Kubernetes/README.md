...
* a Seq server
* a FreeTheIdServer private servers farm
* a FreeTheIdServer public servers farm

By default, all files create entities in **FreeTheIdServer** namespace.
...
* *FreeTheIdServer.pfx* to protect generated keys
...
```bash
kubectl apply -f FreeTheIdServer-certificates-volume.yaml
```
...
```bash
> kubectl describe pvc certificates --namespace=FreeTheIdServer
...
StorageClass:    hostpath
Status:          Bound
Claim:           FreeTheIdServer/certificates
...
* Create a database named *FreeTheIdServer*
* Create a login named *FreeTheIdServer* and map it to *FreeTheIdServer* database db_owner  
...
* Update *FreeTheIdServer-private-connectionstring.yaml* with your base64 connection string. The connection string should resemble *Data Source=sql-server;Initial Catalog=FreeTheIdServer;User Id=FreeTheIdServer;Password={your pwd}*
...
```
kubectl apply -f FreeTheIdServer-private-connectionstring.yaml
```
...
* Update *FreeTheIdServer-private-secrets.yaml* with your base64 encoded passwords.
...
metadata:
  ...
  namespace: FreeTheIdServer
data:
  protect-key: UEBzc3cwcmQ= # replace by your base64 encoded password for FreeTheIdServer.pfx
...
```
kubectl apply -f FreeTheIdServer-private-secrets.yaml
```
...
```
kubectl apply -f FreeTheIdServer-config-volume.yaml
```
...
```bash
kubectl apply -f FreeTheIdServer-private-configmap.yaml
```
...
```bash
kubectl apply -f FreeTheIdServer-private-service.yaml
```
...
* Update *FreeTheIdServer-public-secrets.yaml* with your base64 encoded passwords.

```yaml
kind: Secret
apiVersion: v1
metadata:
  name: FreeTheIdServer-public-secrets
  namespace: FreeTheIdServer
data:
  protect-key: UEBzc3cwcmQ= # replace by your base64 encoded password for FreeTheIdServer.pfx
  tls.pwd: UEBzc3cwcmQ= # replace by your base64 encoded password for tls-private.pfx
...
```
kubectl apply -f FreeTheIdServer-public-secrets.yaml
```
...
```bash
kubectl apply -f FreeTheIdServer-public-configmap.yaml
```
...
```bash
kubectl apply -f FreeTheIdServer-public-keys-volume.yaml
```
...
```bash
kubectl apply -f FreeTheIdServer-public-service.yaml
```

* Create the ingress

```bash
kubectl apply -f FreeTheIdServer-public-ingres.yaml
```
